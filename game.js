...
class Bunker extends GameObject {
    ...
    update(now, allGameObjects, playersState) {
        ...
        // Ensure rally point is valid
        if (!this.rallyPoint || typeof this.rallyPoint !== 'object' || 
            typeof this.rallyPoint.x !== 'number' || typeof this.rallyPoint.y !== 'number') {
            this.rallyPoint = { x: MAP_WIDTH / 2, y: MAP_HEIGHT / 2 };
        }
        ...
        if (timeSinceLastSpawn >= this.spawnCooldown) {
            if (playerState.currentSupply < playerState.supplyCap) {
                ...
                // Send unit spawn to server if multiplayer
                if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                    const unitData = {
                        id: newUnit.id,
                        type: newUnit.type,
                        x: newUnit.x,
                        ...
                        maxHealth: newUnit.maxHealth,
                        movementSpeed: newUnit.movementSpeed,
                        speed: newUnit.speed,
                        size: newUnit.size,
                        supplyCost: newUnit.supplyCost,
                        targetX: newUnit.targetX,
                        targetY: newUnit.targetY,
                        aMoveTargetX: newUnit.aMoveTargetX,
                        aMoveTargetY: newUnit.aMoveTargetY,
                        commandState: newUnit.commandState
                    };
                    window.multiplayerClient.sendAction({
                        type: 'unitSpawned',
                        unit: unitData
                    });
                }
...
class Unit extends GameObject {
    ...
    update(now, allGameObjects) {
        ...
        // Resolve targetUnit if it's an ID string (from multiplayer)
        if (this.targetUnit && typeof this.targetUnit === 'string') {
            const targetObj = allGameObjects.find(obj => obj.id === this.targetUnit);
            if (targetObj && targetObj.health > 0) {
                this.targetUnit = targetObj;
            } else {
                this.targetUnit = null;
        ...
        if (this.targetUnit && typeof this.targetUnit === 'object' && this.targetUnit.health <= 0) {
             this.targetUnit = null;
             if (this.commandState === 'attacking') { this.commandState = 'idle'; }
        }
        
        // Validate targetUnit is an object before using
        if (this.targetUnit && typeof this.targetUnit !== 'object') {
            this.targetUnit = null;
            if (this.commandState === 'attacking') { this.commandState = 'idle'; }
        }
    ...
    handleCombat(now, target) {
        // Safety check: ensure target is a valid object
        if (!target || typeof target !== 'object' || target.health <= 0) {
            this.targetUnit = null;
            this.commandState = 'idle';
            return;
        }
        ...
        if (typeof target.x !== 'number' || typeof target.y !== 'number' || 
            typeof target.size !== 'number' || typeof target.health !== 'number') {
            this.targetUnit = null;
            this.commandState = 'idle';
            return;
        }
...
function handleMouseUp(event) {
    ...
    if (event.button === 0 && buildingPlacementMode && buildingTypeToPlace) {
        ...
        if (isValidPlacement && buildingWorkers.length > 0) {
            ...
            if (firstWorker.build(buildingTypeToPlace, buildingPlacementX, buildingPlacementY, buildingPlacementGridX, buildingPlacementGridY)) {
                ...
                // Send building command to server if multiplayer
                if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode && buildingRef) {
                    // Send building creation to server
                    const buildingData = {
                        id: buildingRef.id,
                        type: buildingRef.type,
                        x: buildingRef.x,
                        y: buildingRef.y,
                        playerId: buildingRef.playerId,
                        health: buildingRef.health,
                        maxHealth: buildingRef.maxHealth,
                        gridX: buildingRef.gridX,
                        gridY: buildingRef.gridY,
                        gridWidth: buildingRef.gridWidth,
                        gridHeight: buildingRef.gridHeight,
                        isUnderConstruction: buildingRef.isUnderConstruction
                    };
                    window.multiplayerClient.sendBuildingCreated(buildingData);
                }
    ...
    if (event.button === 0 && isAMoveMode) {
        ...
        if (commandableUnits.length > 0) {
            const unitIds = commandableUnits.map(u => u.id);
            ...
            // Send attack-move command to server if multiplayer
            if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                window.multiplayerClient.sendAttackMoveCommand(unitIds, mousePos.x, mousePos.y);
            }
        }
        isAMoveMode = false;
        isDragging = false;
...
function handleRightClick(event) {
    ...
    if (commandableUnits.length > 0) {
        ...
        if (clickedBuildingUnderConstruction) {
            ...
            if (clickedTarget) {
                ...
                // Send attack command to server if multiplayer
                if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                    window.multiplayerClient.sendAttackCommand(unitIds, clickedTarget.id);
                }
            } else {
                commandableUnits.forEach(unit => {
                    unit.moveTo(clickPos.x, clickPos.y);
                ...
                issuedMoveCommand = true;
                // Send move command to server if multiplayer
                if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                    window.multiplayerClient.sendMoveCommand(unitIds, clickPos.x, clickPos.y);
                }
            }
        }
    }
    ...
    if (selectedPlayerBunkers.length > 0) {
        if (!clickedTarget) { // Only set rally on ground click
             const bunkerIds = selectedPlayerBunkers.map(b => b.id);
             ...
             // Send rally point command to server if multiplayer
             if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                 window.multiplayerClient.sendRallyPointCommand(bunkerIds, clickPos.x, clickPos.y);
             }
        }
    }
...
window.addEventListener('load', () => {
    ...
    const multiplayerMode = urlParams.get('multiplayer') === 'true' || urlParams.get('mode') === 'multiplayer';
    
    // Initialize multiplayer if requested
    if (multiplayerMode && window.multiplayerClient) {
        const defaultServerUrl = (window.gameConfig && window.gameConfig.SERVER_URL) || 'http://localhost:3000';
        const serverUrl = urlParams.get('server') || defaultServerUrl;
        // Initialize fog of war even in multiplayer
        fogOfWar = new FogOfWar();
        // Initialize UI displays
        lastResourceUpdateTime = performance.now();
        updateResourceSupplyDisplay();
        updateUpgradeLevels();
        // Connect to server - game state will come from server
        window.multiplayerClient.connect(serverUrl);
        // Don't call setupGame() in multiplayer - wait for server state
    } else {
...
function initializeUISystem() {
    ...
    uiSystem.setActionHandler('action-q-p4', () => {
        ...
        if (playerState.resources >= price) {
            ...
            // Send upgrade command to server if multiplayer
            if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                window.multiplayerClient.sendUpgradeCommand('armor');
            }
    ...
    uiSystem.setActionHandler('action-w-p4', () => {
        ...
        if (playerState.resources >= price) {
            ...
            // Send upgrade command to server if multiplayer
            if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                window.multiplayerClient.sendUpgradeCommand('attackDamage');
            }
    ...
    uiSystem.setActionHandler('action-e-p4', () => {
        ...
        if (playerState.resources >= price) {
            ...
            // Send upgrade command to server if multiplayer
            if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                window.multiplayerClient.sendUpgradeCommand('weaponRange');
            }
    ...
    uiSystem.setActionHandler('action-r-p4', () => {
        ...
        if (playerState.resources >= price) {
            ...
            // Send upgrade command to server if multiplayer
            if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                window.multiplayerClient.sendUpgradeCommand('healthRegen');
            }
    ...
    uiSystem.setActionHandler('action-t-p4', () => {
        ...
        if (playerState.resources >= price) {
            ...
            // Send upgrade command to server if multiplayer
            if (window.multiplayerClient && window.multiplayerClient.isMultiplayerMode) {
                window.multiplayerClient.sendUpgradeCommand('movementSpeed');
            }
